...
const LEVELS_CBC = ["Pre School","PP1","PP2","Grade 1","Grade 2","Grade 3","Grade 4","Grade 5","Grade 6","Grade 7","Grade 8","Grade 9","Grade 10","Grade 11","Grade 12"];
...
const SUBS_PRE_LIST = [
  ...
  "Kiswahili Language Activities","English Language Activities",
];
...
const SUBS_CBC = {
  "Pre School":SUBS_PRE_LIST,"PP1":SUBS_PRE_LIST,"PP2":SUBS_PRE_LIST,
...
const toApiPhone = (p) => (p.startsWith("07")||p.startsWith("01")) ? "254"+p.slice(1) : p.startsWith("+254") ? p.slice(1) : p;
const isValidPhone = (p) => /^0[71]\d{8}$/.test(p);
...
const btnPrimary = {
  ...
  animation:"glowOrbit 4s linear infinite, heartbeat 2.6s ease-in-out infinite",
};
...
function Card({ m, getIcon, onPreview, onDownload }) {
  return (
    <div className="glow-border" style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden",display:"flex",alignItems:"stretch"}}>
      <div style={{width:60,flexShrink:0,background:`linear-gradient(180deg,${m.color}cc,${m.color}55)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,position:"relative"}}>
        ...
        <div style={{position:"absolute",bottom:3,fontSize:6,color:"#fff",opacity:0.18,fontStyle:"italic"}}>topplussrevisions.com</div>
      </div>
...
export default function App() {
  ...
  const [justSubUntil, setJustSubUntil] = useState(()=>{
    try{ return Number(localStorage.getItem("just_subscribed_until")||0); }catch(e){ return 0; }
  });
  ...
  // Starts a ~60s "checking subscription" grace window — used right after payment
  // so the UI doesn't flash "No Active Subscription" while the webhook/DB catches up.
  const startSubGrace=()=>{
    const until=Date.now()+60000;
    try{ localStorage.setItem("just_subscribed_until", String(until)); }catch(e){}
    setJustSubUntil(until);
  };

  // Just times out the "checking subscription" grace window — does NOT repeatedly
  // call checkSub, to avoid extra Netlify function invocations. The subscription
  // state is already refreshed once, right when payment is confirmed.
  useEffect(()=>{
    if(!justSubUntil) return;
    const msLeft=justSubUntil-Date.now();
    if(msLeft<=0){
      setJustSubUntil(0);
      try{ localStorage.removeItem("just_subscribed_until"); }catch(e){}
      return;
    }
    const t=setTimeout(()=>{
      setJustSubUntil(0);
    ...
    <nav style={{position:"sticky",top:0,zIndex:200,background:"rgba(8,14,28,0.98)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(255,180,0,0.1)"}}>
      <div style={{padding:"10px 16px 6px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div onClick={()=>setPage("home")} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}>
          <div className="glow-badge" style={{width:32,height:32,background:"linear-gradient(135deg,#ffb400,#ff7b00)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:13,color:"#000",flexShrink:0}}>T+</div>
      ...
      <div style={{padding:"0 16px 10px",display:"flex",alignItems:"center",gap:12,flexWrap:"nowrap",overflowX:"auto"}}>
        <button onClick={()=>setPage("home")} className={page==="home"?"glow-text":""} style={{background:"none",border:"none",color:page==="home"?"#ffb400":"#bbb",cursor:"pointer",fontWeight:600,fontSize:13,padding:0,whiteSpace:"nowrap",flexShrink:0}}>🏠 Home</button>
        <button onClick={()=>setPage("browse")} className={page==="browse"?"glow-text":""} style={{background:"none",border:"none",color:page==="browse"?"#ffb400":"#bbb",cursor:"pointer",fontWeight:600,fontSize:13,padding:0,whiteSpace:"nowrap",flexShrink:0}}>📚 Browse</button>
        ...
        {user?(
          ...
          <div style={{display:"flex",gap:8,flexShrink:0}}>
            ...
            <button onClick={()=>setModal("register")} className="glow-register" style={{background:"linear-gradient(135deg,#ffb400,#ff7b00)",border:"none",color:"#000",padding:"5px 13px",borderRadius:8,cursor:"pointer",fontWeight:800,fontSize:13,whiteSpace:"nowrap"}}>Register</button>
          </div>
        )}
      </div>
    </nav>
  ...
  const ForgotPasswordM=()=>{
    ...
    const send=async()=>{
      ...
      setLd(true);
      const{error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:"https://www.topplussrevisions.com/#reset-password"});
  ...
  const RegisterM=()=>{
    ...
    const go=async()=>{
      ...
      if(!isValidPhone(f.phone)){showToast("Phone must start with 07 or 01","err");return;}
    ...
    return(
      <div>
        ...
        <div style={{display:"grid",gap:11}}>
          ...
          <div><label style={lbl}>Phone (07… or 01…)</label><input value={f.phone} onChange={e=>setF(p=>({...p,phone:e.target.value}))} style={inp} placeholder="0712345678" maxLength={10}/></div>
          ...
          <button onClick={go} disabled={ld} className="glow-register" style={{...btnPrimary,opacity:ld?0.7:1,marginTop:4}}>{ld?"Creating account…":"Register"}</button>
  ...
  const SubscribeM=()=>{
    ...
    const pay=async()=>{
      if(!isValidPhone(phone)){showToast("Enter valid 07 or 01 number","err");return;}
      setLd(true);
      setStep("mpesa");
      try{
      ...
      <div>
        {step==="done"?(
          ...
          <div>
            ...
            <div style={{marginBottom:14}}><label style={lbl}>M-Pesa Phone (07… or 01…)</label><input value={phone} onChange={e=>setPhone(e.target.value)} style={inp} placeholder="0712345678" maxLength={10}/></div>
  ...
  const GateM=()=>(
    <div style={{textAlign:"center",padding:"10px 0"}}>
      ...
      <div style={{display:"grid",gap:10}}>
        <button onClick={()=>setModal("register")} className="glow-register" style={btnPrimary}>Register Free</button>
      ...
      <div>
        ...
        {prevMat.type==="Videos"?(
          ...
          <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:16,marginBottom:12,minHeight:180,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%) rotate(-28deg)",opacity:0.08,fontSize:13,fontWeight:900,color:"#fff",whiteSpace:"nowrap",pointerEvents:"none",letterSpacing:1}}>www.topplussrevisions.com</div>
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          {!user?(<button onClick={()=>setModal("register")} className="glow-register" style={{flex:2,...btnPrimary,padding:"11px 0",borderRadius:9,fontSize:13}}>Register to Continue</button>
  ...
  const AnalyticsTab=()=>{
    const [stats,setStats]=useState({users:0,subscribers:0,biweekly:0,monthly:0,sixmonth:0,annual:0});
    ...
    useEffect(()=>{
      const loadStats=async()=>{
        ...
        const{count:biweekly}=await supabase.from("subscriptions").select("*",{count:"exact",head:true}).eq("plan","biweekly").eq("active",true);
        ...
        setStats({users:users||0,subscribers:subs||0,biweekly:biweekly||0,monthly:monthly||0,sixmonth:sixmonth||0,annual:annual||0});
      };
      loadStats();
    ...
    const revenue=(stats.biweekly*100)+(stats.monthly*200)+(stats.sixmonth*800)+(stats.annual*1200);
    const allStats=[
      ...
      {l:"2-Week Plans",v:stats.biweekly,i:"🗓️"},{l:"Monthly Plans",v:stats.monthly,i:"📆"},{l:"6-Month Plans",v:stats.sixmonth,i:"🗓️"},
  ...
  const Dash=()=>{
    ...
    const checkingSub=!isSubscribed&&!expired&&justSubUntil>Date.now();
    return(
      <div style={{padding:"20px 16px 40px",background:"#080e1c",minHeight:"100dvh"}}>
        ...
        {checkingSub&&(<div style={{background:"rgba(0,229,255,0.06)",border:"1px solid rgba(0,229,255,0.2)",borderRadius:12,padding:"14px",marginBottom:14,textAlign:"center"}}><div style={{fontWeight:700,color:"#00e5ff",marginBottom:5}}>⏳ Checking your subscription…</div><p style={{color:"#aaa",fontSize:13,margin:0}}>This can take up to a minute after payment. No need to pay again.</p></div>)}
        {!isSubscribed&&!expired&&!checkingSub&&(<div style={{background:"rgba(192,57,43,0.1)",border:"1px solid rgba(192,57,43,0.3)",borderRadius:12,padding:"14px",marginBottom:14}}><div style={{fontWeight:700,color:"#e74c3c",marginBottom:5}}>🔒 No Active Subscription</div><p style={{color:"#aaa",fontSize:13,margin:"0 0 10px"}}>Subscribe to preview and download materials.</p><button onClick={()=>setModal("subscribe")} style={{...btnPrimary,padding:"10px 0"}}>Subscribe Now — From KSh 100/2 weeks</button></div>)}
  ...
  const bSubs=filt.level?[...(SUBS_CBC[filt.level]||[]),...(SUBS_844[filt.level]||[])]:([...new Set([...SUBS_PRE_LIST,...SUBS_CBC_LIST,...SUBS_844_LIST])].sort());

  return(
    <div style={{minHeight:"100dvh",background:"#080e1c",color:"#fff",fontFamily:"'DM Sans',sans-serif",overflowX:"hidden",position:"relative"}}>
      ...
      <style>{`
        @keyframes glowOrbit {
          0%   { box-shadow: 8px 0 16px 0 rgba(255,180,0,0.5), -8px 0 16px 0 rgba(255,180,0,0.15); }
          25%  { box-shadow: 0 8px 16px 0 rgba(0,229,255,0.5), 0 -8px 16px 0 rgba(0,229,255,0.15); }
          50%  { box-shadow: -8px 0 16px 0 rgba(255,0,200,0.5), 8px 0 16px 0 rgba(255,0,200,0.15); }
          75%  { box-shadow: 0 -8px 16px 0 rgba(0,229,255,0.5), 0 8px 16px 0 rgba(0,229,255,0.15); }
          100% { box-shadow: 8px 0 16px 0 rgba(255,180,0,0.5), -8px 0 16px 0 rgba(255,180,0,0.15); }
        }
        @keyframes heartbeat {
          0% { transform: scale(1); }
        ...
        @keyframes borderCycle {
          0% { border-color: rgba(255,180,0,0.45); }
          33% { border-color: rgba(0,229,255,0.45); }
          66% { border-color: rgba(255,0,200,0.4); }
          100% { border-color: rgba(255,180,0,0.45); }
        }
        @keyframes textGlow {
          0%,100% { text-shadow: 0 0 6px rgba(255,180,0,0.5), 0 0 14px rgba(255,180,0,0.2); }
          50% { text-shadow: 0 0 10px rgba(0,229,255,0.55), 0 0 20px rgba(0,229,255,0.25); }
        }
        @keyframes waveSpin {
          0% { transform: rotate(0deg) scale(1.1); }
          100% { transform: rotate(360deg) scale(1.1); }
        }
        .glow-btn { animation: glowOrbit 4s linear infinite, heartbeat 2.6s ease-in-out infinite; }
        .glow-register { animation: glowOrbit 4s linear infinite, franticBeat 1.3s ease-in-out infinite; }
        .glow-badge { animation: glowOrbit 4s linear infinite, colorCycle 6s ease-in-out infinite; }
        .glow-border { animation: borderCycle 5s linear infinite; }
        .glow-text { animation: textGlow 3.2s ease-in-out infinite; }
      `}</style>
      <div aria-hidden="true" style={{
        position:"fixed",inset:"-10%",zIndex:0,pointerEvents:"none",overflow:"hidden",
        background:"radial-gradient(ellipse 60% 40% at 30% 20%,rgba(255,180,0,0.10),transparent 60%),radial-gradient(ellipse 55% 35% at 75% 70%,rgba(0,229,255,0.08),transparent 60%),radial-gradient(ellipse 50% 40% at 50% 100%,rgba(255,0,200,0.06),transparent 60%)",
          ...
          <div style={{background:"#080e1c",minHeight:"100dvh"}}>
            <div style={{position:"relative",padding:"44px 20px 48px",textAlign:"center",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10,maxWidth:320,margin:"0 auto 36px"}}>
                ...
                {!user&&<button onClick={()=>setModal("register")} className="glow-register" style={{background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.13)",color:"#fff",padding:"13px 0",borderRadius:10,fontWeight:700,fontSize:14,cursor:"pointer",width:"100%"}}>Register Free</button>}
              </div>
