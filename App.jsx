...
_style.textContent = `
  html, body, #root { background-color: #080e1c !important; margin: 0; }
  html { overscroll-behavior: none; }
  body { min-height: 100dvh; color: #fff; -webkit-font-smoothing: antialiased; }
  #root { min-height: 100dvh; }
  * { box-sizing: border-box; }
`;
document.head.appendChild(_style);
const _tm = document.querySelector('meta[name="theme-color"]') || document.createElement("meta");
_tm.setAttribute("name","theme-color"); _tm.setAttribute("content","#080e1c");
if(!document.querySelector('meta[name="theme-color"]')) document.head.appendChild(_tm);
const _cs = document.querySelector('meta[name="color-scheme"]') || document.createElement("meta");
_cs.setAttribute("name","color-scheme"); _cs.setAttribute("content","dark");
if(!document.querySelector('meta[name="color-scheme"]')) document.head.appendChild(_cs);
    ...
    <div style={{background:"#080e1c",minHeight:"100dvh"}}>
      ...
      {!loading&&topDL.length>0&&(
        <div style={{padding:"0 16px 30px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>{topDL.map(m=><Card key={m.id} m={m}/>)}</div>
        </div>
      )}
      {!loading&&latest.length>0&&(
        <div style={{padding:"0 16px 30px"}}>
          <SectionHead icon="🆕" title="Latest Uploads" sub="Freshly added content"/>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>{latest.map(m=><Card key={m.id} m={m}/>)}</div>
      ...
      {/* Footer — WhatsApp button, no copyright */}
  ...
  const Browse = ()=>{
    ...
    return(
      <div style={{padding:"20px 16px 40px",background:"#080e1c",minHeight:"100dvh"}}>
        ...
        {loading?<div style={{textAlign:"center",padding:"60px 0",color:"#444"}}>⏳ Loading…</div>
        :filtMats.length===0?<div style={{textAlign:"center",padding:"60px 0",color:"#555"}}><div style={{fontSize:36,marginBottom:10}}>🔍</div><div style={{fontSize:14,fontWeight:600}}>No results found</div></div>
        :<div style={{display:"flex",flexDirection:"column",gap:10}}>{filtMats.map(m=><Card key={m.id} m={m}/>)}</div>}
      </div>
    );
  };
        ...
        {tab==="upload"&&(
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:16}}>
            ...
            <div style={{display:"grid",gap:12}}>
              ...
              <div><label style={lbl}>Description</label><textarea value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} style={{...inp,minHeight:72,resize:"vertical",lineHeight:1.5}} placeholder="Brief summary of what this material covers…"/></div>
