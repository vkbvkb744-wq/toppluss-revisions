...
export default function App() {
      ...
      <div>
        ...
        {prevMat.type==="Videos"?(
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,180,0,0.15)",borderRadius:12,padding:24,marginBottom:12,textAlign:"center"}}>
            ...
            <div style={{fontSize:13,fontWeight:700,color:"#fff",marginBottom:4}}>Video not streamed online</div>
            <div style={{fontSize:11,color:"#888",lineHeight:1.6}}>{(isSubscribed||isAdmin)?"Tap Download below to save this video straight to your phone.":"Subscribe, then download this video directly to your phone."}</div>
          </div>
  ...
  const SchoolAdmin=()=>{
    ...
    const [form,setForm]=useState({title:"",description:"",system:"CBC",level:"Grade 1",type:"Notes"});
    const [selectedSubs,setSelectedSubs]=useState([]);
    ...
    const upload=async()=>{
      ...
      if(!form.title){showToast("Fill Title","err");return;}
      if(selectedSubs.length===0){showToast("Select at least one Subject","err");return;}
      if(!file){showToast("Choose a file to upload","err");return;}
      setUploading(true);
      setUploadProgress("Saving…");
      try{
        const{data,error}=await supabase.from("materials").insert([{
          title:form.title,description:form.description,system:form.system,level:form.level,
          subject:selectedSubs[0],subjects:selectedSubs,type:form.type,file_url:null,downloads:0,
        ...
        {tab==="upload"&&(
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:12,padding:16}}>
            <div style={{display:"grid",gap:12}}>
              ...
              <button onClick={upload} disabled={uploading||!active} style={{...btnPrimary,opacity:(uploading||!active)?0.6:1}}>{uploading?(uploadProgress||"Uploading…"):!active?"School Subscription Inactive":"☁ Save & Upload to R2"}</button>
            </div>
          </div>
        )}
