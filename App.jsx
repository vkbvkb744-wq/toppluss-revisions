...
export default function App() {
  ...
  const Card = ({m})=>(
    <div style={{
      ...
      <div style={{
        ...
        <div style={{fontSize:20}}>{icons[m.type]}</div>
      ...
      <div style={{flex:1,padding:"11px 12px",minWidth:0}}>
        ...
        <div style={{display:"flex",gap:7}}>
          <button onClick={()=>{setPrevMat(m);setModal("preview");}} style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.09)",color:"#ccc",borderRadius:8,padding:"7px 0",cursor:"pointer",fontWeight:600,fontSize:11}}>👁 Preview</button>
      ...
      <div style={{padding:"0 16px 10px",display:"flex",alignItems:"center",gap:12,flexWrap:"nowrap",overflowX:"auto"}}>
        ...
        {user ? (
          <button onClick={()=>setPage("dash")} style={{
            ...
            color:"#ffb400",padding:"5px 12px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:12,flexShrink:0,whiteSpace:"nowrap",
          }}>👤 {userName.split(" ")[0]}{isSubscribed&&<span style={{marginLeft:6,fontSize:9,background:"#27ae60",color:"#fff",borderRadius:6,padding:"1px 5px"}}>{subscription.plan}</span>}</button>
        ):(
  ...
  const Home = ()=>(
    <div style={{background:"#080e1c",minHeight:"100vh"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[
            ...
            <div key={plan.name} style={{background:plan.hot?"rgba(255,180,0,0.07)":"rgba(255,255,255,0.03)",border:`1px solid ${plan.hot?"rgba(255,180,0,0.25)":"rgba(255,255,255,0.07)"}`,borderRadius:14,padding:"16px",position:"relative"}}>
              ...
              <button onClick={()=>setModal(user?"subscribe":"register")} style={btnPrimary}>
      ...
      <div style={{background:"#05090f",borderTop:"1px solid rgba(255,255,255,0.05)",padding:"22px 16px 30px",textAlign:"center"}}>
        ...
        <p style={{color:"#444",fontSize:11,margin:"0 0 6px"}}>Kenya's trusted revision platform · CBC & 8-4-4</p>
        <a href="https://wa.me/254755803149" style={{color:"#ffb400",fontSize:12,fontWeight:600,textDecoration:"none"}}>📞 +254 755 803 149</a>
        <p style={{color:"#333",fontSize:10,margin:"6px 0 0"}}>© 2025 Toppluss Revisions</p>
      </div>
    </div>
  );
  ...
  const Admin = ()=>{
    ...
    const upload=async()=>{
      ...
      try{
        ...
        setForm({title:"",system:"CBC",level:"Grade 1",subject:"",type:"Notes"});setFile(null);
        await loadMats();
            ...
            <div style={{display:"grid",gap:12}}>
              ...
              <div>
                ...
                <div onClick={()=>document.getElementById("pdf-in").click()} style={{border:"2px dashed rgba(255,180,0,0.25)",borderRadius:10,padding:"20px",textAlign:"center",cursor:"pointer",background:file?"rgba(39,174,96,0.04)":"transparent"}}>
                  ...
                  :(<><div style={{fontSize:22,marginBottom:4}}>📁</div><div style={{color:"#666",fontSize:13}}>Tap to select PDF</div></>)}
                </div>
      ...
      <div>
        {step==="done"?(
          ...
          <div>
            ...
            <button onClick={pay} disabled={ld} style={{...btnPrimary,opacity:ld?0.7:1}}>{ld?"Sending STK Push…":`Pay ${plan==="weekly"?"KSh 50":"KSh 150"} via M-Pesa`}</button>
          </div>
        )}
      </div>
    );
  ...
  const GateM = ()=>(
    <div style={{textAlign:"center",padding:"10px 0"}}>
      ...
      <p style={{color:"#888",fontSize:13,margin:"0 0 20px",lineHeight:1.65}}>
    ...
    return(
      <div>
        <div style={{display:"flex",gap:12,marginBottom:14,alignItems:"center"}}>
          <div style={{width:52,height:52,background:`linear-gradient(135deg,${prevMat.color}cc,${prevMat.color}44)`,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{icons[prevMat.type]}</div>
          <div>
        ...
        <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:16,marginBottom:12,minHeight:180,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%) rotate(-28deg)",opacity:0.08,fontSize:13,fontWeight:900,color:"#fff",whiteSpace:"nowrap",pointerEvents:"none",letterSpacing:1}}>
          ...
          <div style={{position:"absolute",bottom:0,left:0,right:0,height:60,background:"linear-gradient(transparent,rgba(13,25,41,0.98))",display:"flex",alignItems:"flex-end",justifyContent:"center",paddingBottom:8}}>
            <span style={{fontSize:10,color:"#555",fontStyle:"italic"}}>
              {canSeeMore?"2-page preview — download for full document":"Register free to see more + get 2 free downloads"}
            </span>
          </div>
        </div>
        {!user&&(
        ...
        <div style={{display:"flex",gap:8}}>
          ...
          <button onClick={()=>{setModal(null);user?handleDL(prevMat):setModal("gate");}} style={{flex:2,...btnPrimary,padding:"11px 0",borderRadius:9,fontSize:13}}>
